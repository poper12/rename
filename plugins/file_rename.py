...
def extract_season_episode(filename):
    for pattern, (season_group, episode_group) in SEASON_EPISODE_PATTERNS:
        ...
        if match:
            groups = match.groups()
            try:
                season = match.group(1) if season_group and len(groups) >= 1 else None
                episode = match.group(2) if episode_group and len(groups) >= 2 else (
                    match.group(1) if episode_group and len(groups) >= 1 else None
                )
                logger.info(f"Extracted season: {season}, episode: {episode} from {filename}")
                return season, episode
            except IndexError:
                logger.warning(f"Pattern matched but groups missing in {filename}")
                continue
...
async def auto_rename_files(client, message):
    ...
    try:
        ...
        title, author, caption, thumb = await asyncio.gather(
            codeflixbots.get_title(user_id),
            codeflixbots.get_author(user_id),
            codeflixbots.get_caption(message.chat.id),
            codeflixbots.get_thumbnail(message.chat.id)
        )
        ...
        thumb_cache = globals().get("thumb_cache", {})
        if message.chat.id in thumb_cache:
            thumb_path = thumb_cache[message.chat.id]
        else:
            thumb_path = await process_thumbnail(thumb_path)
            thumb_cache[message.chat.id] = thumb_path
